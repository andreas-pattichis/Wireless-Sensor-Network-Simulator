	...
	public void insertEdge(Node newNode) {
		...
		do {
			...
			while (!currentLevel.isEmpty()) {
				...
				for (int i = 0; i < children.size(); i++) {
					if (newNode.calculateDistance(children.get(i)) < temp.calculateDistance(children.get(i))) {
						newNode.addChild(children.get(i));
						temp.deleteChild(children.get(i));
		...
		for (int i = 1; i < listOfNodes.size(); i++) {
			if (distance.get(i) < minDistance && !newNode.getChildren().contains(listOfNodes.get(i))) {
				idxMinDistance = i;
